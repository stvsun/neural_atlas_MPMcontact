...
class ChartVectorFEMSolver:
    ...
    def __init__(
        ...
        element_order: int = 1,
        jac_sigma_floor: float = 1e-9,
        jac_det_floor: float = 1e-12,
    ):
        ...
        # SVD floors for the stabilized chart-Jacobian pushforward (neural-chart safety)
        self.jac_sigma_floor = jac_sigma_floor
        self.jac_det_floor = jac_det_floor
    ...
    def _precompute_element_quantities(self) -> None:
        ...
        if self.chart_decoder is not None:
            ...
            # consistent physical push-forward for P1 chart gradients.  The inverse and
            # |det| go through stabilized_jacobian_ops (SVD-clamped) so an ill-conditioned
            # NEURAL chart Jacobian cannot blow up the assembly (active-port upgrade).
            self.geom_J = self.decoder_jacobian(self.elem_centroids_ref)
            inv_j, det_abs, kappa, valid = stabilized_jacobian_ops(
                self.geom_J, sigma_floor=self.jac_sigma_floor, det_floor=self.jac_det_floor,
            )
            self.geom_J_inv = inv_j
            self.geom_detJ = torch.det(self.geom_J)   # signed det, kept for diagnostics
            self.geom_cond = kappa                    # per-element condition number
            self.geom_valid = valid                   # per-element well-posedness flag
            ...
            self.vol_phys = self.vol_ref * det_abs    # SVD-clamped |det J|
            ...
            self.geom_cond = torch.ones(self.n_elements, device=self.device, dtype=self.dtype)
            self.geom_valid = torch.ones(self.n_elements, device=self.device, dtype=torch.bool)
