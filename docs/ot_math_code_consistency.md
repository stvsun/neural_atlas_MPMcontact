...
## Status: CONSISTENT (re-verified loop 10; CV-9a mesh-convergence sweep added — centre mean error decreases monotonically 1.22→0.87→0.58→0.52→0.52% over n_rings 6→14, the headline n_rings=10 value 0.58% reproduced EXACTLY at the published config (overlap=0.025, delta=0.03, d4 mesh), every mesh full-Newton converged with force balance ~1e-15; judge-vetted ACCEPT-WITH-FIX ("decreases toward a plateau", all five values listed); no headline number altered. Prior: loop 9 CV-8 five-seed ensemble disclosure; loop 8 coherence; below unchanged)
