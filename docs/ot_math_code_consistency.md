...
## Status: CONSISTENT (re-verified loop 8; FINAL coherence pass — four number-preserving consistency/completeness fixes applied to `paper/main.tex`, no equation moved, no verified number altered, no OT content re-churned; the loop-7 data-extension state below is unchanged)
