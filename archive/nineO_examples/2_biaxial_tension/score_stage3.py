...
def run_score():
    ...
    # --- X8: Nucleation mesh independence (15 pts) ---
    # Uses same BoxDecoder as S1, tight BCs, split into X8a/X8b/X8c
    try:
        ...
        def bc_fn_nuc(nodes_np):
            """Biaxial tension: prescribe u_x=eps*x, u_y=eps*y on faces x=+/-5, y=+/-5;
            u_z=-2*nu/(1-nu)*eps*z on faces z=+/-0.25."""
            n = len(nodes_np)
            u = np.zeros((n, 3))
            mask = np.zeros(n, dtype=bool)
            # Tight face selection
            face_x = np.abs(nodes_np[:, 0]) > 4.99
            face_y = np.abs(nodes_np[:, 1]) > 4.99
            face_z = np.abs(nodes_np[:, 2]) > 0.249
            bnd = face_x | face_y | face_z
            mask[bnd] = True
            u[bnd, 0] = eps_load * nodes_np[bnd, 0]
            u[bnd, 1] = eps_load * nodes_np[bnd, 1]
            u[bnd, 2] = -2 * nu / (1 - nu) * eps_load * nodes_np[bnd, 2]
            return u, mask
        ...
        for nc in [8, 10, 12, 14]:
            dec_c = BoxDecoder(center=(0, 0, 0), half_extents=(5, 5, 0.25)).double()
