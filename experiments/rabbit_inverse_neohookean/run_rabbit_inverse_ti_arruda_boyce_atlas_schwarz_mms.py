#!/usr/bin/env python3
"""
Rabbit TI Arruda-Boyce inverse benchmark using a manufactured solution (MMS).

Key features:
- Fixed rabbit atlas (meshfree coordinate charts).
- Manufactured displacement field u*(x; lambda), with exact body force and traction
  derived from true Arruda-Boyce parameters.
...
def build_run_stem(run_tag: str) -> str:
    ...
    if run_tag:
        return f"rabbit_inverse_ti_arruda_boyce_atlas_schwarz_mms_{run_tag}"
    return "rabbit_inverse_ti_arruda_boyce_atlas_schwarz_mms"
...
def manufactured_displacement(x: torch.Tensor, load_scale: float, amp: float) -> torch.Tensor:
    # Smooth non-axisymmetric displacement field (small-strain by design).
    px = math.pi * x[:, 0]
    py = math.pi * x[:, 1]
    pz = math.pi * x[:, 2]

    ux = amp * load_scale * (
        0.80 * torch.sin(px) * torch.cos(py)
        + 0.20 * torch.sin(py + pz)
    )
    uy = amp * load_scale * (
        0.70 * torch.sin(py) * torch.cos(pz)
        + 0.25 * torch.sin(px + pz)
    )
    uz = amp * load_scale * (
        0.60 * torch.sin(pz) * torch.cos(px)
        + 0.20 * torch.sin(px + py)
    )
...
def manufactured_body_force(
    ...
    div = torch.zeros((x.shape[0], 3), device=x.device, dtype=x.dtype)
    for c in range(3):
        ...
        for j in range(3):
            d = torch.autograd.grad(
                P_true[:, c, j],
                x_var,
                grad_outputs=torch.ones_like(P_true[:, c, j]),
                create_graph=False,
                retain_graph=True,
            )[0][:, j]
            acc = acc + d
...
def manufactured_traction(
    ...
    t_true = torch.bmm(P_true, n_phys.unsqueeze(-1)).squeeze(-1)
    return t_true.detach()
...
def set_u_trainable(u_nets: Sequence[Sequence[LocalVectorPINN]], trainable: bool) -> None:
    for chart in u_nets:
        for net in chart:
...
def compute_level_traction_energy(
    caches: Sequence[ChartCache],
    levels: Sequence[LoadLevel],
    args: argparse.Namespace,
    a0: torch.Tensor,
    ...
    for lidx, lev in enumerate(levels):
        ...
        for i in range(len(caches)):
            t_true = manufactured_traction(
                x=caches[i].x_surf,
                n_phys=caches[i].n_surf,
                load_scale=lev.lam,
                amp=args.mms_disp_amp,
            ...
            arr.append(torch.mean(t_true * t_true).detach().cpu().item())
...
def run_stage(
    ...
    for it in range(1, n_iters + 1):
        ...
        for group in args.color_groups_runtime:
            for i in group:
                ...
                for _ in range(args.local_steps):
                    ...
                    for lidx, lev in enumerate(levels):
                        # interior PDE residual with manufactured body force
                        ...
                        res = torch.nan_to_num(
                            div_flux + det_abs.unsqueeze(-1) * b_true,
                            nan=0.0,
                            posinf=0.0,
                            neginf=0.0,
                        )
                        ...
                        if args.normalize_eq_by_b:
                            b_scale = torch.clamp(
                                torch.mean(torch.sum(b_true * b_true, dim=1)),
                                min=torch.as_tensor(1e-12, device=device, dtype=dtype),
                            )
                        else:
                            b_scale = torch.as_tensor(1.0, device=device, dtype=dtype)

                        if bool(torch.any(valid).item()):
                            loss_eq = loss_eq + torch.mean(res[valid] ** 2) / b_scale
                        else:
                            loss_eq = loss_eq + torch.mean(res**2) / b_scale

                        # surface displacement BC (manufactured)
                        sel_s = sample_indices(c_i.xi_surf.shape[0], args.bc_batch, device=device)
                        xi_s = c_i.xi_surf[sel_s]
                        n_s = c_i.n_surf[sel_s]

                        x_s, xi_s_var, jac_s = chart_map_and_jacobian(
                            decoder=decoders[i],
                            xi_in=xi_s,
                            seed=seeds[i],
                            t1=t1[i],
                        ...
                        u_tar = manufactured_displacement(
                            x_s.detach(),
                            load_scale=lev.lam,
                            amp=args.mms_disp_amp,
                        )
                        loss_trac = loss_trac + torch.mean((u_s - u_tar) ** 2)
        ...
        # checkpoint criteria (parameter recovery priority uses data-mismatch score)
        data_score = data_v if include_data else total_v
    ...
    # For inverse stages we prioritize parameter/data recovery; prefit still uses combo.
    restore_tag = "best_data" if include_data else "best_combo"
    restore_bank = best_data if include_data else best_combo
    load_chart_u_states(u_nets, restore_bank["u"])
    ti_restore(restore_bank["ti"])

    summary = {
        "stage": stage_name,
        "restore_policy": restore_tag,
        "iters": n_iters,
...
def run(args: argparse.Namespace) -> Dict[str, object]:
    ...
    # Manufactured truth snapshots for each chart/load level.
    for i in range(n_charts):
        for lidx, lev in enumerate(levels):
            caches[i].u_teacher_by_level[lidx] = manufactured_displacement(
                x=caches[i].x_surf,
                load_scale=lev.lam,
                amp=args.mms_disp_amp,
            ).detach()
    ...
    # ---------------- Prefit stage (fixed true parameters) ----------------
    hist_prefit = None
    pack_prefit = None
    if args.teacher_iters > 0:
        print(f"Prefit stage: iters={args.teacher_iters}, fixed true parameters")
        hist_prefit, pack_prefit, pair_scores = run_stage(
            stage_name="Prefit",
    ...
    level_energy = compute_level_traction_energy(
        caches=caches,
        levels=levels,
        args=args,
        a0=a0,
    ...
    stage_a_param_only = max(0, min(args.stage_a_param_only_iters, args.stage_a_iters))
    stage_a_joint = max(0, args.stage_a_iters - stage_a_param_only)
    hist_A_parts: List[Dict[str, List[float]]] = []
    pack_A = None

    if stage_a_param_only > 0:
        print(f"Stage A1 (param-only): iters={stage_a_param_only}, unknowns=(mu_m,k_f)")
        set_u_trainable(u_nets, trainable=False)
        hist_A1, pack_A1, pair_scores = run_stage(
            stage_name="StageA1",
            n_iters=stage_a_param_only,
            u_nets=u_nets,
            chart_opts=chart_opts,
            ti_params=ti_params,
        ...
        set_u_trainable(u_nets, trainable=True)
        hist_A_parts.append(hist_A1)
        pack_A = pack_A1

    if stage_a_joint > 0:
        print(f"Stage A2 (joint): iters={stage_a_joint}, unknowns=(mu_m,k_f)+u")
        hist_A2, pack_A2, pair_scores = run_stage(
            stage_name="StageA2",
            n_iters=stage_a_joint,
            u_nets=u_nets,
            chart_opts=chart_opts,
            ti_params=ti_params,
            theta_opt=theta_opt_A,
        ...
        hist_A_parts.append(hist_A2)
        pack_A = pack_A2

    if not hist_A_parts or pack_A is None:
        raise RuntimeError("Stage A produced no iterations. Increase --stage-a-iters.")

    # concatenate A-part histories
    hist_A: Dict[str, List[float]] = {}
    for k in hist_A_parts[0].keys():
        vals: List[float] = []
        for h in hist_A_parts:
            vals.extend(h.get(k, []))
        hist_A[k] = vals
    ...
    for lidx, lev in enumerate(levels):
        ...
        with torch.no_grad():
            ...
            u_s_true = manufactured_displacement(
                x=x_surf,
                load_scale=lev.lam,
                amp=args.mms_disp_amp,
            )
            u_s_true_dom = u_s_true
        ...
        with torch.no_grad():
            ...
            u_d_true = manufactured_displacement(
                x=x_dom,
                load_scale=lev.lam,
                amp=args.mms_disp_amp,
            )
    ...
    metrics = {
        ...
        "mms": {
            "disp_amp": float(args.mms_disp_amp),
        },
        "sensitivity": {
            "init": sens_init,
        ...
        "stages": {
            "prefit": pack_prefit["summary"] if pack_prefit is not None else None,
    ...
    print("Rabbit TI Arruda-Boyce MMS inverse run complete")
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Rabbit TI Arruda-Boyce inverse with manufactured displacement/traction/body-force (MMS)."
    )
    ...
    p.add_argument(
        ...
        default=cfg_get_str(
            cfg,
            "output-dir",
            "/Users/wsun/Documents/Softwares/Mapped_sphere_method_for_complex_geometry/PINN_coordinate_chart_3Dgeometry/runs/rabbit_inverse_ti_arruda_boyce_mms_main",
        ),
    )
    ...
    # Load amplitudes for manufactured displacement family
    ...
    p.add_argument("--mms-disp-amp", type=float, default=cfg_get_float(cfg, "mms-disp-amp", 0.05))

    # Iteration schedules
    p.add_argument("--teacher-iters", type=int, default=cfg_get_int(cfg, "teacher-iters", 50))
    p.add_argument("--stage-a-param-only-iters", type=int, default=cfg_get_int(cfg, "stage-a-param-only-iters", 20))
    ...
    p.add_argument("--normalize-eq-by-b", type=parse_bool, default=cfg_get_bool(cfg, "normalize-eq-by-b", True))
