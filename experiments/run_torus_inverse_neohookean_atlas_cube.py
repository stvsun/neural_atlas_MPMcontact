#!/usr/bin/env python3
"""
Inverse Neo-Hookean on 3D torus using 8 cube charts and mapped Cartesian PDE.

This is a full field inverse PINN variant:
- One displacement PINN per chart u_i(eta), eta in [-1,1]^3.
- Analytic cube->torus chart map with 20% overlap in major-angle coordinate.
...
def chart_eta_from_global(phi: torch.Tensor, s: torch.Tensor, center: torch.Tensor, h_phi: torch.Tensor, eta1_clip: float) -> torch.Tensor:
    ...
    return torch.stack([eta1, s[:, 0], s[:, 1]], dim=1)
...
def chart_map_and_jacobian(
    ...
    eta = eta_in.clone().detach().requires_grad_(True)
...
def build_run_stem(run_tag: str) -> str:
    ...
    if run_tag:
        return f"torus_inverse_neohookean_atlas_cube_{run_tag}"
    return "torus_inverse_neohookean_atlas_cube"
...
def run(args: argparse.Namespace) -> Dict[str, object]:
    ...
    print("Inverse torus cube-chart run complete")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Inverse Neo-Hookean on torus with 8 cube charts.")
