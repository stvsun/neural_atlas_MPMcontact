...
def run_score():
    ...
    # --- X8: Nucleation mesh independence (15 pts) ---
    # Uses same CylinderDecoder as S1, tight BCs, split into X8a/X8b/X8c
    try:
        ...
        def bc_fn_nuc(nodes_np):
            """Indentation: prescribe uniform compression on boundary (eps=1e-3).
            Tight face selection at z < 0.01 and z > L-0.01."""
            n = len(nodes_np)
            u = np.zeros((n, 3))
            ...
            bot = nodes_np[:, 2] < 0.01
            top = nodes_np[:, 2] > (L_block - 0.01)
            bnd = bot | top
            mask[bnd] = True
            # Uniform compression: fix bottom, compress top
            u[top, 2] = -eps_val * L_block
            return u, mask
        ...
        for nc in [6, 8, 10, 12]:
            dec_c = CylinderDecoder(R=R_block, z_center=L_block / 2, L_half=L_block / 2).double()
            ...
            try:
                ...
                e_nl = solve_nonlocal_strain(s_c, e_local, length_scale=1.0)
