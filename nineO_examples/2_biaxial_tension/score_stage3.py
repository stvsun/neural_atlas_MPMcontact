"""Stage 3 XFEM-critic tests for Challenge 2: Biaxial Tension.
...
def run_score():
    ...
    # Build solver with BoxDecoder for biaxial plate
    dec = BoxDecoder(center=(0, 0, 0), half_extents=(5, 5, 0.25)).double()
    ...
    try:
        def sdf_oracle(x):
            """Thin plate: half_extents=(5,5,0.25), negative inside."""
            he = torch.tensor([5.0, 5.0, 0.25], dtype=x.dtype, device=x.device)
            d = torch.abs(x) - he
            return torch.max(d, dim=-1).values

        def bc_fn(nodes_np):
            """Biaxial tension: pull all lateral faces equally."""
            n = len(nodes_np)
            u = np.zeros((n, 3))
            mask = np.zeros(n, dtype=bool)
            eps_val = 1e-3
            left = nodes_np[:, 0] < -4.95
            right = nodes_np[:, 0] > 4.95
            bottom = nodes_np[:, 1] < -4.95
            top = nodes_np[:, 1] > 4.95
            bnd = left | right | bottom | top
            mask[bnd] = True
            u[bnd, 0] = eps_val * nodes_np[bnd, 0]
            u[bnd, 1] = eps_val * nodes_np[bnd, 1]
            return u, mask

        res = test_nucleation_mesh_independence(
            decoder_cls=BoxDecoder,
            decoder_args={"center": (0, 0, 0), "half_extents": (5, 5, 0.25)},
