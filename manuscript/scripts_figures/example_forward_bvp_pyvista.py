...
This script no longer reruns the atlas solve. It reconstructs the saved
multi-chart torus state from a real checkpoint, exports raw per-chart VTK data
for debugging, exports a deduplicated physical point cloud for displacement
visualization, and renders the manuscript figures from the reconstructed fields.
"""
...
def main():
    ...
    panels = [
        ...
        ("eig_diff_Sp", r"$\lambda^p_1 - \lambda^p_3$", "turbo", "%.4f"),
    ]
    ...
    fig.tight_layout(pad=0.3)
    for ext in ("png", "pdf"):
        fig.savefig(OUT_DIR / f"forward_bvp_Fp_fields.{ext}", bbox_inches="tight", dpi=200)
    plt.close(fig)
