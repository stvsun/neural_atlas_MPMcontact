...
INTERP_SURFACE = CACHE_DIR / "example2_rabbit_interpolated.vtp"
FIG_PATH = OUT_DIR / "example2_rabbit_poisson_pub.png"
FIG_PDF = OUT_DIR / "example2_rabbit_poisson_pub.pdf"
...
def load_or_build_surface() -> pv.PolyData:
    ...
    points = pv.read(BOUNDARY_RAW)
    if RECON_SURFACE.exists():
        surface = pv.read(RECON_SURFACE)
    else:
        surface = pv.PolyData(points.points).reconstruct_surface(nbr_sz=20, sample_spacing=0.01)
        RECON_SURFACE.parent.mkdir(parents=True, exist_ok=True)
        surface.save(RECON_SURFACE)

    interpolated = surface.interpolate(points, radius=0.03, sharpness=2.0)
...
def render_panel(mesh: pv.DataSet, scalar: str, cmap: str, clim: tuple[float, float], out_path: Path) -> None:
    ...
    # Use the Stanford bunny Y-up convention so the head reads at the top
    # and the tail sits lower in the frame.
...
def build_figure() -> None:
    ...
    pred_img_path = CACHE_DIR / "example2_pred.png"
    err_img_path = CACHE_DIR / "example2_err.png"
    entropy_img_path = CACHE_DIR / "example2_entropy.png"
...
def main() -> None:
    parser = ArgumentParser(description="Generate publication-ready rabbit Poisson figure.")
    parser.parse_args()
    build_figure()
